...
#define MASK_RX_DR(x) (uint8_t(uint8_t(x)<<6))
#define MASK_TX_DS(x) (uint8_t(uint8_t(x)<<5))
#define MASK_MAX_RT(x) (uint8_t(uint8_t(x)<<4))
#define EN_CRC(x)		(uint8_t(uint8_t(x)<<3))
#define CRCO(x)		   (uint8_t(uint8_t(x)<<2))
#define PWR_UP(x)  	   (uint8_t(uint8_t(x)<<1))
#define PRIM_RX(x)			(uint8_t(uint8_t(x)<<0))

//RF SETUP REGISTER BIT ACCESS

#define PLL_CLOCK_MASK (0x10)
#define PLL_CLOCK_UNMASK (0x00)
#define RF_DR_MASK(x) (uint8_t(uint8_t(x)<<3))
#define RF_PWR_18 (0X00)
#define RF_PWR_12 (0X01)
#define RF_PWR_6 (0X02)
#define RF_PWR_0 (0X00)
#define LNA_HCURR(x) (uint8_t(uint8_t(x)<<0))

//STATUS REGISTER BIT ACCESS
#define RX_DR (uint8_t(uint8_t(1)<<6))
#define TX_DS (uint8_t(uint8_t(1)<<5))
#define MAX_RT (uint8_t(uint8_t(1)<<5))

//RF_CH REGISTER BIT ACCESS
#define RFCH(x) (uint8_t(x))

//FIFO_STATUS_BIT_ACCESS
#define TX_REUSE (uint8_t(FIFO_STATUS)&&uint8_t(0x40))
